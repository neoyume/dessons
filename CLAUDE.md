...
- **Pipette de calque** : `process()` calcule `pr/pg/pb` (RGB ajusté contraste+saturation, `adjustRGB()`) en plus de `lum`/`hue`. En `sepMode='pick'`, `layerOf(px)` = plus proche voisin RGB parmi `layers.map(L=>hexToRgb(L.color))`. UI : `pickLayer` (index armé, -1 = aucun) ; clic sur la pastille (devenue `<button>`) arme, clic sur `#preview` échantillonne (`imgCtx.getImageData` + `adjustRGB` — on échantillonne ce qui est *affiché*, pas le pixel brut) et écrit dans `L.color`/`L.pickColor`. `L.pickColor` survit aux changements de `sepMode` (mémorisé même en `hue`/`lum`) ; `rebuildLayers()` le restaure en repassant sur `pick`.
...
- **Séparation « par couleurs » avec pipette** (`sepMode='pick'`) : clique la pastille d'un calque → armé → clique l'image → couleur échantillonnée devient la cible du calque ; affectation par plus proche voisin RGB.
