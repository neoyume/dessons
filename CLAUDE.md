...
- **`playDir` et `curve` = boutons-icônes** (pas des `<select>`) rendus par `renderTransportIcons()` à côté du bouton « Écouter l'aperçu » (panneau gauche). L'état vit dans deux `<input type="hidden">` (`#playDir`, `#curve`) → tous les `$('playDir').value` / `$('curve').value` marchent tels quels. Clic direction → `process()` ; clic courbe → `redrawPreview()`. Icônes SVG `currentColor` : flèches pour la direction, 6 ticks placés par `curveFrac` pour la courbe (l'icône EST la groove). Re-render sur changement de langue (tooltips).
...
- JS, dans l'ordre :
  ...
  8. Transport & sorties : un seul moteur (scheduler + tête de lecture), deux **sinks** via la variable `sink` :
     ...
     - `sink === 'synth'` — **synthé d'aperçu Web Audio** (zéro dépendance) : bouton « Écouter l'aperçu » (panneau de gauche). `ensureAudio()` (crée l'`AudioContext` au 1er clic + compresseur + petite réverb `makeImpulse`), `timbreFor(prog)` (famille GM → forme d'onde + ADSR + filtre), `spawnVoice()` (osc + biquad + enveloppe, `+sub` pour les basses, plafond 96 voix), `stopAllVoices()`. L'horloge audio est verrouillée sur `performance.now` à chaque tick du scheduler (`audioEpoch`).
       **iOS Safari** : le handler de clic de `#previewBtn` est **volontairement synchrone** (pas `async`/`await`) — `ensureAudio()` crée le contexte *et* démarre un buffer muet d'1 sample de façon synchrone dans le même geste utilisateur (le vrai déblocage audio iOS), puis `startPlayback('synth')` enchaîne aussitôt sans attendre `ac.resume()` (appelé en fire-and-forget). Ne jamais réintroduire un `await` entre le clic et le premier `osc.start()` — WebKit peut sortir du contexte de geste et couper le son silencieusement. Séparément : le switch sonnerie/silence de l'iPhone/iPad coupe la sortie Web Audio de Safari, rien à faire côté JS.
