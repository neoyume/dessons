...
- **Split into layers**: separate the image by color (hue), by brightness (shadows / midtones / highlights), or **by picked colors** — click a layer's swatch (it outlines and pulses), then click anywhere on the preview to set that layer's target color from the image; every pixel is then assigned to its nearest picked color. Each layer becomes its own MIDI track, with its own General MIDI instrument and name. Each row also has **M** (mute) and **S** (solo) toggles to A/B layers while listening — they affect the preview and Web MIDI only; the `.mid` export always keeps every layer.
...
- **Playback direction & curve** (icon buttons next to the ▶ preview button):
  - *Direction* — left→right (default), right→left, top→bottom or bottom→top. Which axis carries time and which way the playhead sweeps it.
...
- **Preview sound**: the **▶ Preview sound** button plays the loop right in the browser through a small built-in synth (rough approximations of the General MIDI families) — no DAW, no setup, works in every browser including Safari and iOS/iPadOS. If you hear nothing on iPhone/iPad, check the physical ringer/silent switch — Safari mutes Web Audio output when it's set to silent.
