...
- **Playback direction & curve** (icon buttons next to the ▶ preview button):
  ...
  - *Curve* — time-warps the playhead across the loop: *linear* (steady), *accelerating*, *decelerating*, *pulsing* (dense on the beat, sparse mid-loop) or *swing*. Same note pattern, different grooves. The curve icon itself shows the timing; faint ticks along the leading edge of the preview show it at full resolution.
  - Both apply to the preview, real-time playback **and** the `.mid` export.
