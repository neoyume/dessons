...
- **Playback curve**: time-warps the playhead across the loop — *linear* (steady), *accelerating*, *decelerating*, *pulsing* (dense on the beat, sparse mid-loop) or *swing*. Turns the same note pattern into different grooves. The faint ticks along the leading edge of the preview show where each step lands in time. Baked into the export too.
